...
function DockSocialMediaLinks() {
    ...
    const socialLinks = [
        ...
        { name: 'TikTok', url: tiktok, icon: Music, color: 'hover:bg-black' },
        { name: 'Telegram', url: telegram, icon: MessageCircle, color: 'hover:bg-blue-500' },
    ...
    return (
        <div className={`fixed z-50 flex flex-col gap-3 ${isRTL ? 'left-6 bottom-6' : 'right-6 bottom-6'
            }`}>
                    ...
                    {socialLinks.map((link, index) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {/* Tooltip */}
                                <div className={`absolute px-3 py-2 bg-gray-900 text-white text-sm rounded-lg 
                                              opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none
                                              whitespace-nowrap shadow-lg ${isRTL
                                        ? 'left-full ml-3'
                                        : 'right-full mr-3'
                                    }`}>
                                    {link.name}
                                    <div className={`absolute top-1/2 transform -translate-y-1/2 w-0 h-0 
                                                  border-t-4 border-t-transparent border-b-4 border-b-transparent ${isRTL
