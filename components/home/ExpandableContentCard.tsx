...
import { Calendar, Clock, MapPin, Users, User, BookOpen, Star, ArrowRight, X } from 'lucide-react'
...
const ExpandableContentCard: React.FC<ExpandableContentCardProps> = ({ items, type, className = '' }) => {
    ...
    return (
        <>
                        ...
                        >
                            ...
                            <motion.div layoutId={`image-${active.title}-${id}`} className="relative flex-shrink-0">
                                ...
                                <div className="absolute top-2 left-2 sm:top-4 sm:left-4">
                                    <Badge className={`${getCategoryColor(active.category)} border text-xs sm:text-sm`}>
                                        {active.category}
                                    </Badge>
                                </div>
                            </motion.div>
            ...
            {/* Image Grid */}
            <div className={`grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2 sm:gap-3 md:gap-4 ${className}`}>
                {items.map((item, ) => (
                    <motion.div
                    ...
                    >
                        {/* Image */}
                        <motion.div layoutId={`image-${item.title}-${id}`} className="relative w-full h-full">
                            <Image
                                src={item.image}
                                alt={item.title}
                                fill
                            ...
                            <div className="absolute top-1 left-1 sm:top-2 sm:left-2">
                                <Badge className={`${getCategoryColor(item.category)} text-xs border-0`}>
